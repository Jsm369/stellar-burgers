...
export const OrderInfoUI: FC<
  OrderInfoUIProps & { isModal?: boolean; numberTitle?: string }
> = memo(({ orderInfo, isModal, numberTitle }) => (
...
//   OrderInfoUIProps & { isModal?: boolean; numberTitle?: string }
// > = memo(({ orderInfo, isModal, numberTitle }) => {
//   console.log('Order status:', orderInfo.status); // Проверим, что статус приходит правильно
...
//         {orderInfo.name}
//       </h3>
//       <OrderStatus status={orderInfo.status} /> {/* Передаем статус */}
//       <p className={`text text_type_main-medium pt-15 pb-6`}>Состав:</p>
//       <ul className={`${styles.list} mb-8`}>
//         {Object.values(orderInfo.ingredientsInfo).map((item, index) => (
//           <li className={`pb-4 pr-6 ${styles.item}`} key={index}>
